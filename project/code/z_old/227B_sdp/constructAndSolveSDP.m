...
for i=1:num_c
    ...
    for j=1:num_local_assgn_Ci
        ...
        RLS((i-1) * M + j) = constraint_i.evaluate(x);
    end
end
            ...
            for l=1:D
                for v2 = constraints{ c_i }.scope
                    for l2 = 1 : D
                        mlvl = ML(c_i, :, v, l);
                        mlv2l2 =  ML(c_i, :, v2, l2);
                        bothml = intersect(find(mlvl), find(mlv2l2));
                        indicies = (c_i-1)*M + bothml;
                        if (~isempty(indicies))
                            sum(lambdaVar(indicies)) == sigmaVar( (  v - 1) * D + l, ( v2 - 1 ) * D + l2 );
                        else
...
cvx_solver_settings('dumpfile','~/Documents/Classes/EE227C/project/code/tempproblem.mat');    
