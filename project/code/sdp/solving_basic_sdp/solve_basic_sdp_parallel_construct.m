function sol = solve_basic_sdp_parallel_construct( varargin )
%
% ***************************** usage *************************************
%
% This function constructs Basic SDP for an input CSP in SDPNAL+ format,
% and then solves that SDP with SDPNAL+. Use this function when you have
% access to the Matlab Parallel Computing Toolbox and you want to construct
% a large SDP.
%
% If you do not have access to Matlab's Parallel Computing Toolbox, use
% "solve_basic_sdp" instead.
%
% ************************** signature ************************************
%
...
[obj,X,~,~,~,~,~,~,info,runhist] = ...
    ...
    if(strcmp(ME.message(1:10),'mexbwsolve') || strcmp(ME.message(1:10),'mexfwsolve'))
        ...
        warning(char(str));
        [AL, AS, b] = remove_ld_rows(At{1}', At{2}', b);
        At{1} = AL';
        At{2} = AS';
        [obj,X,~,~,~,~,~,~,info,runhist] = ...
            sdpnalplus(blk,At,C,b,L,U,Bt,l,u,OPTIONS);
