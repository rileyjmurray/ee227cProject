...
\begin{frame}{Useful Lemma's}
	\begin{lem}
		For every $x$, $\sum_{_y \in X} \dist{x-y}^2 = \Delta_1^2(X) + n \dist{x - c}^2$.\\
		Hence, $\sum_{ \{x,y\} \subseteq X } \dist{x-y}^2 = n \Delta_1^2(X)$.
	\end{lem}
	\begin{center}
		\begin{tikzpicture}
		% def loc second cluster
		\def\clusterX{5};
		\def\clusterY{1.5};
		\node[small node] (x11) at (0,0) {};
		\node[small node] (x12) at (1.5,0) {};
		\node[small node] (x13) at (0.75,-0.75) {};
		\node[small node] (x14) at (0.75,0.75) {};
		
		
		\node[special node] (x) at ( \clusterX, \clusterY) {$x$};
		...
		\visible<2>{
			\node[center node] (c) at ( 0.75,0) {$c$};
			
			\draw[edge] (x) to node[above]{$4\times $} (c);
			\draw[edge] (c) to (x11);
			\draw[edge] (c) to (x12);
			\draw[edge] (c) to (x13);
			\draw[edge] (c) to (x14);
